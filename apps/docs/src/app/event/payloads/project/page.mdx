...
| `deployment_url` | `string`       | O        | 배포 URL (없으면 `null`)                    |
| `club`         | `object`         | O        | 소속 동아리 `{ club_id, name }` (없으면 `null`) |
| `participants` | `array<object>`  | X        | 참여자 목록 `[{ student_number, name }]`     |
| `repositories` | `array<string>`  | X        | 리포지토리 URL 목록 (없으면 `[]`)                 |
| `tech_stacks`  | `array<string>`  | X        | 기술 스택 목록 (없으면 `[]`)                     |

`club`은 `{ club_id, name }`, `participants`의 학생은 `{ student_number, name }`만 담습니다.
`repositories`와 `tech_stacks`는 문자열 배열이며, 등록된 값이 없으면 빈 배열로 옵니다.
  ...
  "data": {
    ...
    "new": [
      { "index": 0, "object": {
        ...
        "deployment_url": "https://datagsm.kr",
        "club": { "club_id": 1, "name": "더모먼트" },
        "participants": [ { "student_number": 2106, "name": "김부원" } ],
        "repositories": [ "https://github.com/themoment-team/datagsm-server" ],
        "tech_stacks": [ "Kotlin", "Spring Boot" ]
      } }
    ]
  }
}
  ...
  "data": {
    "old": [
      { "index": 0, "object": {
        ...
        "repositories": [ "https://github.com/themoment-team/old-portal" ],
        "tech_stacks": [ "Java" ]
      } }
    ],
    "new": [
